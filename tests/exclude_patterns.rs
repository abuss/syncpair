...
async fn test_exclude_patterns_functionality() -> Result<()> {
    ...
    assert!(
        excluded_count > 0,
        "Exclude patterns should filter out some files"
    );

    // Verify specific files are excluded
    let filtered_paths: Vec<&String> = filtered_files.iter().map(|f| &f.path).collect();
    assert!(
        !filtered_paths.iter().any(|p| p.ends_with(".tmp")),
        ".tmp files should be excluded"
    );
    assert!(
        !filtered_paths.iter().any(|p| p.ends_with(".log")),
        ".log files should be excluded"
    );
    assert!(
        !filtered_paths.iter().any(|p| p.ends_with(".bak")),
        ".bak files should be excluded"
    );
    assert!(
        !filtered_paths.iter().any(|p| p.contains("node_modules")),
        "node_modules should be excluded"
    );
    assert!(
        !filtered_paths.iter().any(|p| p.contains(".git")),
        ".git directory should be excluded"
    );
    assert!(
        !filtered_paths.iter().any(|p| p.contains("temp/")),
        "temp/ directory should be excluded"
    );

    // Verify normal files are included
    assert!(
        filtered_paths.iter().any(|p| p.contains("normal_file.txt")),
        "normal files should be included"
    );
    assert!(
        filtered_paths.iter().any(|p| p.contains("README.md")),
...
async fn test_exclude_patterns_edge_cases() -> Result<()> {
    ...
    assert!(
        filtered_paths.iter().any(|p| p.contains("file.TMP")),
        "file.TMP should be included (case sensitive)"
    );
    assert!(
        filtered_paths.iter().any(|p| p.contains("temporary.txt")),
        "temporary.txt should be included"
    );
    assert!(
        !filtered_paths.iter().any(|p| p.contains("document.tmp")),
        "document.tmp should be excluded"
    );
