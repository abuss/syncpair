...
async fn test_basic_sync() -> Result<()> {
    ...
    assert_eq!(
        std::fs::read_to_string(synced_file)?,
        content,
...
async fn test_conflict_resolution() -> Result<()> {
    ...
    assert_eq!(
        std::fs::read_to_string(client_a_dir.join(file_name))?,
        "Content from B"
    );
    assert_eq!(
        std::fs::read_to_string(client_b_dir.join(file_name))?,
        "Content from B"
    );
...
async fn test_deletion_propagation() -> Result<()> {
    ...
    // Verify deletion
    assert!(
        !client_b_dir.join(file_name).exists(),
        "File should be deleted on Client B"
    );
...
async fn test_delta_sync() -> Result<()> {
    ...
    assert_eq!(
        ...
        "Server content should match client content after delta sync"
    );
