...
impl MultiDirectoryClient {
    pub fn new(config: ClientConfig) -> Result<Self> {
        ...
        for dir_config in &config.directories {
            if !dir_config.settings.enabled {
            ...
            info!("Configured directory '{}': {}", 
                  dir_config.name, 
                  dir_config.local_path.display());
        }
        
        if clients.is_empty() {
            return Err(anyhow::anyhow!("No enabled directories found in configuration"));
        }
        
        info!("MultiDirectoryClient initialized with {} directories", clients.len());
    ...
    pub async fn start_watching_with_shutdown(&self, shutdown_rx: Option<broadcast::Receiver<()>>) -> Result<()> {
        info!("Starting multi-directory synchronization for client: {}", self.config.client_id);
        ...
        for ((dir_name, client), shutdown_tx) in self.clients.iter().zip(shutdown_channels.iter()) {
            ...
            let task = tokio::spawn(async move {
                ...
                match client_clone.start_watching_with_shutdown(Some(shutdown_rx)).await {
                    Ok(_) => {
                        info!("File watcher stopped successfully for directory: {}", dir_name_clone);
                        Ok(())
                    }
                    Err(e) => {
                        error!("Error in file watcher for directory {}: {}", dir_name_clone, e);
                        Err(e)
                    }
                }
            });
    ...
    pub fn get_enabled_directories(&self) -> Vec<&DirectoryConfig> {
        self.config.directories
            .iter()
            .filter(|dir| dir.settings.enabled)
            .collect()
    }
